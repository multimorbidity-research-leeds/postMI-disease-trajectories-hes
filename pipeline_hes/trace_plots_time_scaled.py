...
def display_categories(fig,gs,labelsIdx,colours,textCol):
    ...
    plt.title('ICD-10\ncategories',fontsize=RR_FIG_TITLE_FONTSIZE)
    ax.margins(0.1,0)


def display_traces_time_scaled(fig,gs,df_rmst,chapter_mapping,
                               labelsIdx,colours,textCol):
    ...
    plt.title('RMST-scaled disease trajectory', fontsize=RR_FIG_TITLE_FONTSIZE)
    plt.xlabel('RMST (years)', fontsize=8)
    ax.set_ymargin(0.01)
...
def plot_rr_fig_time_scaled(df_rr):
    ...
    # Categories
    display_categories(fig,gs[0,-1],labelsIdx,colours,textCol)
    plt.title('E\n', loc='left',fontsize=RR_FIG_TITLE_FONTSIZE_LETTER)
