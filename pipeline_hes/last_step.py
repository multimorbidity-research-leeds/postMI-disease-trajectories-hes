...
def count_diagnosis_appearances_groupby_high(traces_high,traces_low):
    ...
    for i in np.unique(allD_g.index.get_level_values(0)):
        ...
        tmp = tmp.rename(columns={'index':'ICD-10 diagnosis',
                                  'low':'Occurrences'})
        all_tables.append(tmp[['ICD-10 diagnosis','Occurrences','prcSTR_withinChapter']])
        #print(tmp)
    return all_tables
...
def main():
    ...
    #%% For shiny - counts of 3-char at each trace position
    ...
    for (k,v) in datasets.items():
        ...
        last_step.for_shiny_disease_counts(variants_controls_per_subject, \
                                 variants_patients_per_subject,
                                 list(df_rr_hr.index),filename)
        df_rr_hr.to_csv(os.path.join(params.DIR_RESULTS,'df_rr_hr_{}.csv'.format(k)), index=True)
    ...
    #%% TABLE A - trace NUM EVENTS vs DURATION 
    ...
    #%% TABLE B - counts of ICD-10 3-char per chapter
    ...
    #%% TABLE C - freq of chapter appearances
    ...
    tmp_mask= pd.concat([
        variants_patients_per_subject['variant']=='INIT,I00-I99,F01-F99,Censor',
        variants_patients_per_subject['variant']=='INIT,I00-I99,I00-I99,F01-F99,Censor',
        variants_patients_per_subject['variant']=='INIT,I00-I99,I00-I99,I00-I99,F01-F99,Censor'],axis=1).any(axis=1)
    tmpCirc = variants_patients_per_subject.loc[tmp_mask].copy()
    ...
    trace_mask_anyENDO = \
        df_rr_hr.index.map(
        lambda trace: not re.search('INIT,.*((E00-E89,.*I00-I99)|(I00-I99,.*E00-E89))+,.*',
    ...
    trace_mask_any_BEGIN_etc = \
        df_rr_hr.index.map(
        lambda trace: not re.search('INIT,((A00-B99)|(D50-D89)|(E00-E89)|(F01-F99))+,.*',
                                    trace) is None).values.astype(bool)
    ...
    trace_mask_tmp = \
        df_rr_hr.index.map(
        lambda trace: not re.search('INIT,((F01-F99,)|(I00-I99,))*((I00-I99,F01-F99,)|(F01-F99,I00-I99,))((F01-F99,)|(I00-I99,))*Censor',
                                    trace) is None).values.astype(bool)
    print(df_rr_hr.loc[trace_mask_tmp][['RR','RR_CIl','RR_CIu','HR','HR_CIl','HR_CIu']].sort_values('HR'))    

    #%% TABLE (digestive, top ones)
    
    print('In interesting traces, look at 3-char ICD-10 frequency')
    
    # MORE FREQUENT, DIGESTIVE
    interesting_traces = df_rr_hr.sort_values('RR').iloc[-18:].merge(
        variants_patients_per_subject,
 ...
 #   interesting_traces.groupby('variant').apply(lambda x: count_diagnosis_appearances(x['variant_ALT'],lowlevel=True))
    ...
    df_adj_p = df_adj_p.drop(columns=['nomatch'])
    ...
    df_adj_c = df_adj_c.drop(columns=['nomatch'])
    ...
    importlib.reload(plot_adj_graph)
    plot_adj_graph.plot_graph_granular_for_visjs_javascript(df_adj_p.values,df_adj_p.index.values,
                              'MI subjects',variants_patients_count_alt['count'].sum(),
                              'Initial MI')
    plot_adj_graph.plot_graph_granular_for_visjs_javascript(df_adj_c.values,df_adj_c.index.values,
                              'Controls',variants_controls_count_alt['count'].sum(),
                              'Initial Diagnosis')
